...
df = pd.read_excel(
    BASE_DIR / f"급여정산({YEAR}-{MONTH:02}).xlsm", sheet_name="mailsend", header=0
)
...
for i in range(len(df)):
    try:
        ...
        mail["Subject"] = f"{YEAR}년 {MONTH}월 PROJECT-N 급여명세서입니다. (재발송)"
        mail["Cc"] = "wjpark@project-n.or.kr"

        msg = MIMEText(
            f"""{YEAR}년 {MONTH}월 PROJECT-N 급여명세서입니다.

명세서에 직급이 정상적으로 표시되지 않아 수정 후 재발송합니다.
...
감사합니다."""
        ...
        file.add_header(
            "Content-Disposition", "attachment", filename=f"{df.iloc[i][3]}"
        )
        mail.attach(file)
